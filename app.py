...
__version__ = "1.0 (2022-10-09) (Release)"
__version_simple__ = 2
...
def reloadData():
    global users, configs, folderInfo, filetypes, rpath, loginSessionLists, ipAccessControl
    users = json.load(open("configs/users.json", "r", encoding="utf-8"))
    configs = json.load(open("configs/configs.json", "r", encoding="utf-8"))
    folderInfo = json.load(
        open("configs/folderInfo.json", "r", encoding="utf-8"))
    loginSessionLists = json.load(
        open("configs/loginSessions.json", "r", encoding="utf-8"))
    ipAccessControl = json.load(
        open("configs/ipAccessControl.json", "r", encoding="utf-8"))
...
for lang in supportLang:
    langData[lang] = json.load(
        open("translates/" + lang + ".json", encoding="utf-8"))


def gstr(strid):
    userLang = request.cookies.get("lang")
    if userLang == None or userLang not in supportLang.keys():
        userLang = request.accept_languages.best_match(supportLang.keys(),
                                                       "en_US")
...
def isFolder(path):
    return os.path.isdir(os.path.join(rpath, path))


app.jinja_env.globals.update(gstr=gstr,
...
def newDownloadFolderProcess(doId, path, zipfilepath):
    ...
    downloadFolderProcess[doId] = {
        "totalSize": sum(fileSizeList),
        "currentDoSize": currentDoSize,
        "total": total,
        "currentZip": currentDoCount,
        "zipFilePath": zipfilepath,
        "doId": doId,
        "stop": False,
        "folderPath": path
...
def getFolderSize(path):
    ...
    return sizeof_fmt(
        sum(f.stat().st_size for f in root_directory.glob('**/*')
            if f.is_file()))
...
def login(usrList, rememberMe=False, saveToSessionLists=True):
    ...
    loginSession = "".join([
        random.choice(string.ascii_letters + string.digits) for n in range(128)
    ])
    session["loginSession"] = loginSession
    if saveToSessionLists:
        loginSessionLists[loginSession] = {"userId": usrList["userId"]}
    open("configs/loginSessions.json", "w",
         encoding="utf-8").write(json.dumps(loginSessionLists))
    reloadData()
    userClass = User()
...
def before_req():
    if request.remote_addr != "127.0.0.1":
        if ipAccessControl["mode"] == 0:  #IP Ban Mode
            if request.remote_addr in ipAccessControl["ips"].keys():
                return abort(403)
        else:  #IP Allow Mode
            if request.remote_addr not in ipAccessControl["ips"].keys():
                return abort(403)
    ...
    if account != None and password != None:
        for user in users:
            if users[user]["name"] == account and check_password_hash(
    ...
    if action == "getResource":
        ...
        if rfile in "script.js":
            return render_template_string(
                open("resources/" + rfile, encoding="utf-8").read())
        if rfile in "folderView.js":
            return render_template_string(open("resources/" + rfile,
                                               encoding="utf-8").read(),
                                          users=users)
        return send_file("resources/" + rfile)
    elif action == "changeIPAccessControlMode":
        ipAccessControl["mode"] = int(
            request.form.get("whitelistMode", 0) == "on")
        open("configs/ipAccessControl.json",
             "w").write(json.dumps(ipAccessControl))
        reloadData()
        return redirect("?action=serverControlPanel&lastScrollY=" +
                        request.args.get("lastScrollY", "0"))
        ...
        if current_user.role != "admin":
            return abort(401)
        resp = Response(
            '<meta http-equiv="refresh" content="0; url=/?action=serverControlPanelAfterRestart" />'
        )
        ...
        if request.method == "POST":
            ...
            if request.files:
                ...
                if file != []:
                    ...
                    for file in file.values():
                        cpath = ""
                        for fpath in os.path.dirname(
                                pathList[findex]).split("/"):
                            cpath += fpath + "/"
                            if os.path.isdir(os.path.join(freqpath,
                                                          cpath)) == False:
        ...
        zipfilepath = os.path.join(
            configs["folderArchivePath"],
            reqPath.split("/")[-1] + dt.now().strftime("-%Y-%m-%d-%H-%M-%S") +
            ".zip")
        Thread(target=newDownloadFolderProcess,
               args=(doId, reqPath, zipfilepath)).start()
        ...
        if request.method == "POST":
            ...
            for user in users:
                if users[user]["name"] == formUser and check_password_hash(
                        users[user]["password"], password):
        ...
        if session.get("loginSession", "") in loginSessionLists.keys():
            ...
            open("configs/loginSessions.json",
                 "w").write(json.dumps(loginSessionLists))
            reloadData()
            session.pop("loginSession")
        logout_user()
        return redirect(request.args.get("next", "/"))
    elif action == "canDelete":
        return str(int(checkUserCanDelete(reqPath)))
    elif action == "getUserPermission":
        userPermission = []
        for user in users:
            user = users[user]
            userId = user["userId"]
            userPermission.append({
                ...
                userId,
                "canAccess":
                checkUserCanAccess(reqPath, userId),
                "canUpload":
                checkUserCanUpload(reqPath, userId),
                "canDelete":
                checkUserCanDelete(reqPath, userId)
            })
        return json.dumps(userPermission)
    elif action == "delete":
        if request.method == "POST":
        ...
        if request.method == "POST":
            if checkUserCanUpload(reqPath):
                if os.path.isdir(freqpath) == False:
                    return abort(404)
                newFolderName = request.form.get(
                    "newFolderName", request.args.get("newFolderName", ""))
        ...
        if request.args.get("from", "") == "serverControlPanel":
            return redirect("/?action=serverControlPanel")
        if os.path.isfile(freqpath):
            return redirect("/".join(request.path.split("/")[:-1]))
        return redirect(request.path)
    elif action == "removeFilesOptions":
        if current_user.is_authenticated == False:
            return redirect("/?action=login&next=" + request.path)
        if current_user.role != "admin":
            return abort(401)
        ...
        if request.method == "POST":
            if current_user.is_authenticated == False:
                return redirect(
                    "/?action=login&next={}?action=adminManageAccount".format(
                        reqPath))
            if current_user.role != "admin":
                return abort(401)
            if request.args.get("userId", "") in users.keys():
                users.pop(request.args.get("userId", ""))
                for f in glob("configs/avatars/{}.*".format(
        ...
        if request.method == "POST":
            ...
            users[usrId]["name"] = request.form.get("username",
                                                    current_user.name)
            if request.form.get("newPassword", users[usrId]["password"]) != "":
                users[usrId]["password"] = generate_password_hash(
                    request.form.get("newPassword", users[usrId]["password"]))
            open("configs/users.json", "w").write(json.dumps(users))
            reloadData()
            flash(gstr("changesSaved"), category="success")
            return redirect("/?action=serverControlPanel")
        return render_template("adminManageAccount.html",
                               user=users[request.args.get(
                                   "userId", str(current_user.id))])
    elif action == "manageAccount":
        if current_user.is_authenticated == False:
            return redirect(
                "/?action=login&next={}?action=manageAccount".format(reqPath))
        if request.method == "POST":
            users[str(current_user.id)]["name"] = request.form.get(
                "username", current_user.name)
            if request.form.get("newPassword", users[str(
                    current_user.id)]["password"]) != "":
                users[str(
                    current_user.id)]["password"] = generate_password_hash(
                        request.form.get(
                            "newPassword",
                            users[str(current_user.id)]["password"]))
            open("configs/users.json", "w").write(json.dumps(users))
            reloadData()
            flash(gstr("changesSaved"), category="success")
            return redirect(reqPath)
        ...
        if current_user.role != "admin":
            return abort(401)
        return render_template("serverControlPanel.html",
                               serverName=configs["serverName"],
                               users=users,
                               serverVersion=__version__,
                               serverVersionSimple=__version_simple__,
                               osVersion=platform.platform(),
                               configVersion=configs["configVersion"],
                               folderInfo=folderInfo.keys(),
                               ipAccessControl=ipAccessControl)
    elif action == "deleteIPControl" and request.method == "POST":
        ipAccessControl["ips"].pop(request.args.get("ip"), None)
        open("configs/ipAccessControl.json",
             "w").write(json.dumps(ipAccessControl))
        reloadData()
        return redirect("?action=serverControlPanel")
    elif action == "addIPControl" and request.method == "POST":
        ipAccessControl["ips"][request.form.get("ip", None)] = "bruh"
        open("configs/ipAccessControl.json",
             "w").write(json.dumps(ipAccessControl))
        reloadData()
        return redirect("?action=serverControlPanel")
        ...
        if request.method == "POST":
            users[configs["nextUserId"]] = {
                "userId": configs["nextUserId"],
        ...
        if request.args.get("userId", None) == None:
            if current_user.is_authenticated == False:
                return send_file("resources/defaultAvatar.png",
                                 mimetype="image/png")
            avatarFile = glob("configs/avatars/" + str(current_user.id) + ".*")
        else:
            if request.args.get("userId") not in users.keys():
                return abort(404)
            avatarFile = glob("configs/avatars/" +
                              request.args.get("userId", "") + ".*")
        if len(avatarFile) == 0:
            return send_file("resources/defaultAvatar.png",
                             mimetype="image/png")
        ...
        if current_user.is_authenticated == False:
            if request.args.get("userId", None) == None:
                ...
                return redirect(
                    "/?action=login&next=?action=adminManageAccount&userId=" +
                    request.args.get("userId", "1"))
        if request.args.get("userId",
                            None) != None and current_user.role != "admin":
            return abort(401)
        if request.args.get("userId", None) != None and request.args.get(
                "userId", None) not in users.keys():
            return redirect("/?action=serverControlPanel")
        ...
        if request.method == "POST":
            ...
            for ofile in glob("configs/avatars/" + uid + ".*"):
                os.remove(ofile)
            file.save("configs/avatars/" + uid + "." +
                      file.filename.split(".")[-1])
            im = Image.open("configs/avatars/" + uid + "." +
                            file.filename.split(".")[-1])
        ...
        if request.method == "POST":
            ...
            im = Image.open("configs/serverLogo." +
                            file.filename.split(".")[-1])
            width, height = im.size
            newLength = min(width, height)
            left = (width - newLength) / 2
            ...
            configs[
                "serverLogoPath"] = "configs/serverLogo." + file.filename.split(
                    ".")[-1]
            open("configs/configs.json", "w").write(json.dumps(configs))
            reloadData()
            #flash(gstr("fileUploaded"),category="success")
        ...
        if oneTimeAccessToken != "" and oneTimeAccessToken in oneTimeAccess.keys(
            ...
            if oneTimeAccess[oneTimeAccessToken]["expire"] >= time.time():
                ...
                userClass.name = users[str(
                    oneTimeAccess[oneTimeAccessToken]["userId"])]["name"]
    ...
    if os.path.isdir(freqpath):
        ...
        if configs["findFolderIndex"] and os.path.isfile(
                os.path.join(freqpath, "index.html")):
            if checkUserCanAccess(reqPath + "/index.html"):
                return open(os.path.join(freqpath, "index.html"),
                            "r",
                            encoding="utf-8").read()
            else:
                return abort(401)
        ...
        for file in files:
            ...
            fileInfo["modifyTime"] = dt.fromtimestamp(
        ...
        if reqFormat == "json":
            ...
            filesDict["serverInfo"] = {
                "serverTime": dt.now().strftime("%Y-%m-%d %H:%M:%S"),
                "serverName": configs["serverName"]
            }
            return json.dumps(filesDict)
        elif reqFormat == "print":
            return render_template("printFileListsPage.html",
                                   files=filesDict,
                                   reqFolder=(reqPath.split("/")[-1]))
    else:
        pass


if configs["webServer"]["useSSL"] == True:
    app.run(host=configs["webServer"]["host"],
            port=configs["webServer"]["port"],
            debug=configs["webServer"]["debug"],
            ssl_context=(configs["webServer"]["ssl"]["cert"],
                         configs["webServer"]["ssl"]["key"]),
            threaded=True)
else:
    app.run(host=configs["webServer"]["host"],
            port=configs["webServer"]["port"],
